...
const jwtAdminAuth = async (req, res, next) => {
    ...
    try{
        ...
        const admin = await Admin.findOne({_id : verified.id}); //This is the data that is fetched from the database.

        if(!admin) return res.status(404).json({error : 'Admin not found'}); //Response that the server sends back to the client if there is an error.
